...
public class TournamentManagement {
    ...
    void viewTournamentDetails() {
        try (Connection con = Databaseconnection.getConnection()) {
            String query = "SELECT t.tournament_id, t.name ,t.start_date, t.end_date, t.status, t.Max_allowed, t.Organizer_id ,v.name AS venue_name FROM tournaments as t LEFT JOIN venue v ON t.venue_id = v.venue_id";
            ...
            while (rs.next()) {
                ...
                System.out.printf("| %-13d| %-18s| %-15s| %-15s| %-12s| %-12d| %-12d| %-20s|\n",
                        tournament_id, name, startDate, endDate, status, maxAllowed, organizerId,venue_name);
            }
            System.out.println(
                    "+------------------------------------------------------------------------------------------------------------------------------------+");
        }
